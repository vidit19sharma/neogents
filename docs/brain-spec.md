...
| `FRICTION.md` | Append-only ledger of corrections, review blocks, and escalations, with stable `[F-NNN]` IDs. Evidence for improving NEO itself. | When the session hits friction: a user correction, a smith BLOCK, a failed-fix escalation, a revert after approval. |
...
- Append-only. Never edited, never rewritten. When enough entries implicate the same component, that's the evidence for changing it.
...
4. neo-shadow writes: rewrites `ACTIVE.md`, appends `PROGRESS.md`, updates `DECISIONS.md`/`LESSONS.md` only if the delta earns it, appends `FRICTION.md` when the delta contains a correction, review block, or escalation, touches `ARCHITECTURE.md` only on pattern changes, `BRIEF.md` only on pivots, and updates `WORKFLOWS.md` when the session repeated a known multi-step sequence.
...
Compaction is covered deterministically: `checkpoint.sh` snapshots git state and the last assistant message to `.neo/CHECKPOINT.md` on PreCompact and (throttled) on Stop, with no LLM in the loop. The curated save still happens at task boundaries.
