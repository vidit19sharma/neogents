...
| 18 | Lesson GC (2026-08-20) | Lessons rot (files move, patterns retire) and rot looks plausible — it needs evidence, not similarity checks. Split the job: `brain-gc.sh` deterministically flags `DEAD-ANCHOR` / `AGED` / `UNDATED` entries at zero LLM cost; `/neo:gc` has neo-shadow review only the flagged entries — KEEP / REWRITE / ARCHIVE to `.neo/brain/archive/LESSONS.md`, never delete outright. Prevention over cure: entries are dated, anchored, and superseding on write so rot can't accumulate. |
| 19 | Deterministic harness layer (2026-08-20) | Deterministic work never goes to an LLM. The advisory `save-brain.sh` asked the model to save at PreCompact — auto-compaction never gives it a turn, so it saved nothing. Replaced with `checkpoint.sh` (PreCompact + throttled Stop → git state + last assistant message to `.neo/CHECKPOINT.md`, re-injected at load when fresh). Subagent output that only lives in chat context dies at compaction, so `run-ledger.sh` (PostToolUse on Agent/Task) writes every spawn to `.neo/runs/` as files. `FRICTION.md` records corrections/blocks/escalations with stable `[F-NNN]` IDs; `/neo:recall` greps the memory instead of re-exploring; `/neo:evolve` turns friction clusters into ONE bounded, human-gated edit — never to `hooks/` or CI, because enforcement stays outside the loop it enforces. |
